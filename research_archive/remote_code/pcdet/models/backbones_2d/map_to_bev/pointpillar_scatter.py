...
def _deduplicate_pillars_keep_last(indices, pillars):
    """Make scatter indices unique while preserving last-write-wins semantics."""
    if indices.numel() < 2:
        return indices, pillars
    sorted_indices, order = torch.sort(indices, stable=True)
    keep = torch.ones_like(sorted_indices, dtype=torch.bool)
    keep[:-1] = sorted_indices[:-1] != sorted_indices[1:]
    return sorted_indices[keep], pillars[:, order[keep]]
    ...
    def forward(self, batch_dict, **kwargs):
        ...
        for batch_idx in range(batch_size):
            ...
            batch_indices = torch.nonzero(
                coords[:, 0] == batch_idx, as_tuple=False
            ).flatten()
            this_coords = coords.index_select(0, batch_indices)
            indices = this_coords[:, 1] + this_coords[:, 2] * self.nx + this_coords[:, 3]
            indices = indices.type(torch.long)
            pillars = pillar_features.index_select(0, batch_indices)
            pillars = pillars.t()
            indices, pillars = _deduplicate_pillars_keep_last(indices, pillars)
            spatial_feature.index_copy_(1, indices, pillars)
