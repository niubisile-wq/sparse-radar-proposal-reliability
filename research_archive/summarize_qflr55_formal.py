...
def read_ap(dataset: str, seed: int):
    paths = sorted(
        LOGS.glob(f"eval_rdar_qflr55_{dataset}_seed{seed}_gpu*.log")
    )
    if not paths:
        return None
...
for dataset, baseline_by_seed in BASELINE.items():
    ...
    complete = all(value is not None for value in aps.values())
    deltas = {
    ...
    ci_lower = mean - T_CRIT_95_DF2 * std / math.sqrt(len(values))
    passed = (
        all(delta > 0 for delta in values)
        and mean >= 1.0
        and ci_lower > 0
    )
    all_pass &= passed
...
print(f"qflr55_all_datasets_formal_gate={all_pass}")
