...
class ATSSTargetAssigner(object):
    ...
    """
    def __init__(self, topk, box_coder, match_height=False, use_multihead=False):
        self.topk = topk
    ...
    def assign_targets_single(self, anchors, gt_boxes, gt_classes):
        ...
        # Force at least one anchor for every GT.  The upstream vectorized
        # assignment is invalid when several sparse-radar GT boxes share the
        # same best anchor: duplicate advanced indices can leave label/value
        # tensors inconsistent.  Resolve collisions by taking the best unused
        # anchor for each GT.
        sorted_iou, sorted_anchor_idx = ious.sort(dim=0, descending=True)
        used_anchor_idx = set()
        for gt_idx in range(num_gt):
            for rank_idx in range(num_anchor):
                anchor_idx = int(sorted_anchor_idx[rank_idx, gt_idx].item())
                if anchor_idx not in used_anchor_idx:
                    anchors_to_gt_indexs[anchor_idx] = gt_idx
                    anchors_to_gt_values[anchor_idx] = sorted_iou[rank_idx, gt_idx]
                    used_anchor_idx.add(anchor_idx)
                    break

        valid_mask = anchors_to_gt_values != INF
        safe_gt_indices = anchors_to_gt_indexs.clamp(min=0, max=num_gt - 1)
        cls_labels = gt_classes.new_zeros((num_anchor,))
        cls_labels[valid_mask] = gt_classes[safe_gt_indices[valid_mask]]
        matched_gts = gt_boxes[safe_gt_indices]

        pos_mask = valid_mask & (cls_labels > 0)
        ...
        if pos_mask.sum() > 0:
            ...
            reg_targets[pos_indices] = self.box_coder.encode_torch(
                matched_gts.index_select(0, pos_indices),
                anchors.index_select(0, pos_indices)
            )
            reg_weights[pos_indices] = 1.0
