...
status="$log_dir/wave_four_modules_seed${seed}.status"
mkdir -p "$log_dir"
...
run_experiment() {
    ...
    shift 3
    local tag="fair_four_modules_${dataset}_seed${seed}"
...
run_experiment 0 astyx "$cfg_dir/pointpillars_four_modules_astyx_car.yaml" \
...
run_experiment 1 truckscenes "$cfg_dir/pointpillars_four_modules_truckscenes_car.yaml" \
...
run_experiment 2 v2xradarv "$cfg_dir/pointpillars_four_modules_v2xradarv_car.yaml" \
    --set DATA_CONFIG.DATASET UnifiedRadarDataset \
...
run_experiment 3 kradar "$cfg_dir/pointpillars_four_modules_kradar_car.yaml" \
    --set DATA_CONFIG.DATASET UnifiedRadarDataset \
