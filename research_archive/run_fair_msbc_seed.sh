...
status="$log_dir/wave_msbc_seed${seed}.status"
mkdir -p "$log_dir"
export PYTHONPATH="$repo${PYTHONPATH:+:$PYTHONPATH}"
cd "$repo"

run_experiment() {
    local gpu="$1" dataset="$2" cfg="$3"
    shift 3
    local tag="fair_bevgate_dapg_msbc_${dataset}_seed${seed}"
...
run_experiment 0 astyx "$cfg_dir/pointpillars_bevgate_dapg_msbc_astyx_car.yaml" \
    --set DATA_CONFIG.DATASET AstyxDataset DATA_CONFIG.DATA_PATH "$repo/data/astyx" \
    DATA_CONFIG.INFO_PATH.train "['astyx_infos_train.pkl']" \
    DATA_CONFIG.INFO_PATH.test "['astyx_infos_val.pkl']" &
run_experiment 1 truckscenes "$cfg_dir/pointpillars_bevgate_dapg_msbc_truckscenes_car.yaml" \
    --set DATA_CONFIG.DATASET UnifiedRadarDataset \
...
run_experiment 2 v2xradarv "$cfg_dir/pointpillars_bevgate_dapg_msbc_v2xradarv_car.yaml" \
    --set DATA_CONFIG.DATASET UnifiedRadarDataset \
...
run_experiment 3 kradar "$cfg_dir/pointpillars_bevgate_dapg_msbc_kradar_car.yaml" \
    --set DATA_CONFIG.DATASET UnifiedRadarDataset \
