...
for iou in 0.30 0.35 0.40 0.45; do
  for strength in 0.25 0.40 0.50 0.60 0.75; do
    tag="vote_iou${iou/./p}_s${strength/./p}"
    ...
    env CUDA_VISIBLE_DEVICES="$gpu" "$py" "$root/evaluate_box_voting.py" \
      ...
      --vote_iou "$iou" --strength "$strength" --residual_count 50 \
