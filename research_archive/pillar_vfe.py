...
class PillarVFE(VFETemplate):
    def __init__(self, model_cfg, num_point_features, voxel_size, point_cloud_range, grid_size=None, **kwargs):
        ...
        self.use_doppler_reliability_gate = self.model_cfg.get(
            'USE_DOPPLER_RELIABILITY_GATE', False
        )
        ...
        self.use_pillar_attention = self.model_cfg.get('USE_PILLAR_ATTENTION', False)
        self.attention_hidden_ratio = max(1, int(self.model_cfg.get('ATTENTION_HIDDEN_RATIO', 4)))
        self.attention_residual = self.model_cfg.get('ATTENTION_RESIDUAL', True)
        ...
        if self.use_doppler_reliability_gate and not self.use_velocity_decomposition:
            raise ValueError(
                'USE_DOPPLER_RELIABILITY_GATE requires '
                'USE_VELOCITY_DECOMPOSITION'
            )
        if self.use_doppler_reliability_gate:
            gate_hidden = int(self.model_cfg.get('DOPPLER_GATE_HIDDEN', 16))
            self.doppler_gate_max_range = float(
                self.model_cfg.get('DOPPLER_GATE_MAX_RANGE', 60.0)
            )
            self.doppler_gate_rcs_scale = float(
                self.model_cfg.get('DOPPLER_GATE_RCS_SCALE', 20.0)
            )
            self.doppler_gate_velocity_scale = float(
                self.model_cfg.get('DOPPLER_GATE_VELOCITY_SCALE', 10.0)
            )
            self.doppler_gate_dispersion_scale = float(
                self.model_cfg.get('DOPPLER_GATE_DISPERSION_SCALE', 5.0)
            )
            self.doppler_reliability_gate = nn.Sequential(
                nn.Linear(5, gate_hidden),
                nn.GELU(),
                nn.Linear(gate_hidden, 1),
            )
            # The complete gate is 1 + tanh(delta). Zero initialization makes
            # the initial network exactly equal to fixed Doppler vectorization.
            nn.init.zeros_(self.doppler_reliability_gate[-1].weight)
            nn.init.zeros_(self.doppler_reliability_gate[-1].bias)
        else:
            self.doppler_reliability_gate = None
        ...
        if self.feature_order is not None:
            ...
            num_point_features += int(self.model_cfg.get('EXTRA_INPUT_FEATURES', 0))
        ...
        if self.use_pillar_attention:
            pillar_channels = self.num_filters[-1]
            hidden_channels = max(pillar_channels // self.attention_hidden_ratio, 8)
            self.pillar_attention = nn.Sequential(
                nn.Linear(pillar_channels, hidden_channels, bias=False),
                nn.ReLU(inplace=True),
                nn.Linear(hidden_channels, pillar_channels, bias=True),
            )
        else:
            self.pillar_attention = None
    ...
    def forward(self, batch_dict, **kwargs):
        ...
        if self.use_velocity_decomposition:
            ...
            if self.use_doppler_reliability_gate:
                valid_mask = self.get_paddings_indicator(
                    voxel_num_points, base_features.shape[1], axis=0
                ).to(base_features.dtype)
                valid_count = voxel_num_points.to(base_features.dtype).view(
                    -1, 1
                ).clamp(min=1)
                masked_velocity = v_comp * valid_mask
                velocity_mean = (
                    masked_velocity.sum(dim=1, keepdim=True) / valid_count
                )
                velocity_variance = (
                    ((v_comp - velocity_mean) * valid_mask).square().sum(
                        dim=1, keepdim=True
                    )
                    / valid_count
                )
                velocity_dispersion = torch.sqrt(
                    velocity_variance.clamp(min=0.0) + 1e-6
                ).expand_as(v_comp)

                radial_range = torch.sqrt(
                    base_features[:, :, 0].square()
                    + base_features[:, :, 1].square()
                    + 1e-6
                )
                range_feature = torch.log1p(radial_range) / max(
                    torch.log1p(
                        radial_range.new_tensor(self.doppler_gate_max_range)
                    ).item(),
                    1e-6,
                )
                rcs_feature = torch.tanh(
                    base_features[:, :, self.rcs_index]
                    / max(self.doppler_gate_rcs_scale, 1e-6)
                )
                speed_feature = torch.tanh(
                    v_comp.abs() / max(self.doppler_gate_velocity_scale, 1e-6)
                )
                density_feature = (
                    torch.log1p(valid_count)
                    / torch.log1p(valid_count.new_tensor(32.0))
                ).expand_as(v_comp)
                dispersion_feature = torch.tanh(
                    velocity_dispersion
                    / max(self.doppler_gate_dispersion_scale, 1e-6)
                )
                ...
                reliability = 1.0 + torch.tanh(
                    self.doppler_reliability_gate(gate_inputs)
                )
                velocity_comp = velocity_comp * reliability
            if self.normalize_velocity_comp:
        ...
        if self.pillar_attention is not None:
            attention = torch.sigmoid(self.pillar_attention(features))
            if self.attention_residual:
                features = features * (1.0 + attention)
            else:
