...
stage="$root/results/m3_consensus_stage_${dataset}_seed${seed}.pkl"
output="$root/results/m3_consensus_${dataset}_seed${seed}.pkl"
stage_log="$root/logs/fair_ablation/m3_consensus_stage_${dataset}_seed${seed}_gpu${gpu}.log"
final_log="$root/logs/fair_ablation/m3_consensus_${dataset}_seed${seed}_gpu${gpu}.log"
...
env CUDA_VISIBLE_DEVICES="$gpu" "$py" "$root/evaluate_expert_quality_gate.py" \
  ...
  --match_iou 0.50 --alpha 0.30 --iou_power 0.00 \
  --unmatched_scale 0.75 --residual_count 50 \
...
env CUDA_VISIBLE_DEVICES="$gpu" "$py" "$root/evaluate_box_voting.py" \
  ...
  --vote_iou 0.25 --strength 0.45 --mode xy --residual_count 50 \
